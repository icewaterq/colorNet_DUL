...
class Trainer(BaseTrainer):
    ...
    def step(self, epoch, batch_in, train=False, visualise=False, save_batch=False, writer=None, tag="train"):
        ...
        #     img3 = F.grid_sample(frames1[0], tf, align_corners=False, mode="nearest")
        #
        #
        #     img1 = frames1[0,i,:].permute(1,2,0).numpy()
        #     img2 = frames2[0,i,:].permute(1,2,0).numpy()
        #     img3 = img3[i,:].permute(1,2,0).numpy()
        #     img1 = np.array((img1+2)*64,dtype=np.uint8)
        #     img2 = np.array((img2+2)*64,dtype=np.uint8)
        #     img3 = np.array((img3+2)*64,dtype=np.uint8)
        ...
        # print('frames1',frames1.size())
        # print('frames2', frames2.size())
        # print('images1', images1.size())
        # print('images2', images2.size())
        # print('affine1', affine1.size())
        # print('affine2', affine2.size())
...
def train(args, cfg):
    ...
    for epoch in range(trainer.start_epoch, cfg.TRAIN.NUM_EPOCHS + 1):
        ...
        if epoch % cfg.LOG.ITER_VAL == 0:
            trainer.checkpoint_best(1, epoch, 'res4')
            trainer.best_score = 0
            trainer.checkpoint_best(1, epoch, 'key')
            trainer.best_score = 0
...
#pytorch-1.6
# python infer_vos.py   --cfg configs/ytvos.yaml --exp 0001 --run final --resume snapshots\YT_0001\0001\ytvos_e060_res4.pth --infer-list filelists\val_davis2017_test --mask-output-dir .\output --seed 0 --set TEST.KEY res4
# python trainmy.py --cfg configs/ytvos.yaml --exp 0002 --run 0002 --seed 27

# python evaluation_method.py --task semi-supervised --davis_path /home/jykj/disk4T/dataset/DAVIS --results_path /home/jykj/disk4T/whq/dino-main/output
# python evaluation_method.py --task semi-supervised --davis_path /home/jykj/disk4T/dataset/DAVIS --results_path /home/jykj/disk4T/whq/dense-ulearn-vos-main/output/res4_vos/