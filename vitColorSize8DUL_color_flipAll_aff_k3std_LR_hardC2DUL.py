...
class ViT(nn.Module):
    def __init__(self, cfg, ohem_range=[0.4, 1.0],is_thin = False):
        super().__init__()
    ...
    def forward(self, frames1, frames2, affines, affine_src, labLst, w_mask_lst, rgbs1, rgbs2, frames1_d, frames2_d, affines_d, affine_src_d):
        result = {}
        result['feat1'] = None
        result['feat2'] = None
...
if __name__ == '__main__':
    ...
    net = ViT(cfg, ohem_range=ohem_range,is_thin=is_thin).cuda()
    net.cuda()
    print(net)
    ...
    for epoch in range(1, 301):
        ...
        for i, batch in enumerate(dataloader):
            ...
            optimzer.zero_grad()
            if True:
                pred_result = net(frames1, frames2, affine1, affine_src, labs1, w_mask_lst, rgbs1, rgbs2, frames1_d, frames2_d, affine1_d, affine_src_d)
        ...
        if epoch % 20 == 0:
            if is_thin:
                torch.save(net.convlocal.state_dict(),
                           './snapshots/DULs8_My01_thin_{}.pkl'.format(epoch))
            else:
                torch.save(net.convlocal.state_dict(),
                           './snapshots/DULs8_My01_{}.pkl'.format(epoch))
