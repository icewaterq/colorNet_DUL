    ...
    def __call__(self, images, masks, affine=None):
        ...
        for k, image in enumerate(images):
            ...
            if s <= 1.:
                ...
                image_pad = F.pad(image, (pad_l, pad_t, pad_r, pad_b), padding_mode='reflect')
    ...
    def __call__(self, images, masks):
        ...
        for k, (image, mask) in enumerate(zip(images, masks)):
            ...
            if random.random()<0.7:
                ...
                image = image.crop((sx,sy,sx+w,sy+h))
                mask = mask.crop((sx,sy,sx+w,sy+h))
            images[k] = image.resize((self.size[1],self.size[0]))
            masks[k] = mask.resize((self.size[1],self.size[0]))
        return images, masks
